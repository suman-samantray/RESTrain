...
print  >> ouf, "[ pairtypes ]"
for i in range(ipairtypes_start+1,ibondtypes_start-1):
    line=lines[i][:-1]
    if line[0]==';':
        print >> ouf, line
        continue
...
for i in range(icmaptypes_start+2,imoletype_start):
    if lines[i]=='\n':
        print >> ouf
        continue
    if lines[i][0]=='C':
...
atom_types=[]
#print >> ouf, '[ atoms ]'
for i in range(iatoms+1,iatoms+671):
...
idihedrals=i+1
for i in range(idihedrals,idihedrals+10000):
...
iimp=i+2
print >> ouf, '[ dihedrals ]'
for i in range(iimp,iimp+100000):
