#!/opt/local/bin/python

import sys, numpy

def get_dihparam(type1,type2,type3,type4,dih_dict):

    if dih_dict.has_key((type1,type2,type3,type4)):
        return dih_dict[(type1,type2,type3,type4)]
    if dih_dict.has_key((type4,type3,type2,type1)):
        return dih_dict[(type4,type3,type2,type1)]
    if dih_dict.has_key(('X',type2,type3,'X')):
        return dih_dict[('X',type2,type3,'X')]
    if dih_dict.has_key(('X',type3,type2,'X')):
        return dih_dict[('X',type3,type2,'X')]

def get_impparam(type1,type2,type3,type4,imp_dict):

    if imp_dict.has_key((type1,type2,type3,type4)):
        return imp_dict[(type1,type2,type3,type4)]
    if imp_dict.has_key((type1,'X','X',type4)):
        return imp_dict[(type1,'X','X',type4)]
    if imp_dict.has_key((type4,'X','X',type1)):
        return imp_dict[(type4,'X','X',type1)]

    return 0,0

inf=open(sys.argv[1],'r')
ouf=open(sys.argv[2],'w')
scale=float(sys.argv[3])
rscale=numpy.sqrt(scale)

lines=inf.readlines()
for i in range(len(lines)):
    lines[i]=lines[i].strip()

## find important bits in input top file

atom_types={}
pair_types={}
bond_types={}
constraint_types={}
angle_types={}
dihedral_types={}
improper_types={}
implicit_genborn_params={}
cmap_types={}

iatomtypes_start=lines.index('[ atomtypes ]')
inbtypes_start=lines.index('[ nonbond_params ]')
ipairtypes_start=lines.index('[ pairtypes ]')
iconstypes_start=lines.index('[ constrainttypes ]')
ibondtypes_start=lines.index('[ bondtypes ]')
iangletypes_start=lines.index('[ angletypes ]')
idihtypes_start=lines.index('[ dihedraltypes ]')
imoletype_start=lines.index('[ moleculetype ]')

## write out header information (unaltered)
for i in range(iatomtypes_start):
    print >> ouf, lines[i]

## edit atomtypes section
print  >> ouf, "[ atomtypes ]"
for i in range(iatomtypes_start+1,inbtypes_start-1):
    line=lines[i]
    if line[0]==';':
        print >> ouf, line
        continue
    print line
    data=line.split()
    print data
    print len(data)
    aname=data[0]
    atnum=int(data[1])
    mass=float(data[2])
    charge=float(data[3])
    ptype=data[4]
    sig=float(data[5])
    eps=float(data[6])
    print >> ouf, aname,atnum,mass,charge,ptype,sig,eps
    print >> ouf, aname+'_',aname,mass,charge,ptype,scale*sig,scale*eps

## edit nonbond_params section
print >> ouf, "[ nonbond_params ]"
for i in range(inbtypes_start+1,ipairtypes_start-1):
    line=lines[i]
    print line
    if line=='':
        continue
    if line[0]==';':
        print >> ouf, line
        continue

    data=line.split()
    aname1=data[0]
    aname2=data[1]
    sig=float(data[3])
    eps=float(data[4])
    print >> ouf, aname1,aname2,1,sig,eps
    print >> ouf, aname1,aname2+'_',1,rscale*sig,rscale*eps
    print >> ouf, aname1+'_',aname2,1,rscale*sig,rscale*eps
    print >> ouf, aname1+'_',aname2+'_',1,scale*sig,scale*eps


## edit pairtypes section
print  >> ouf, "[ pairtypes ]"
for i in range(ipairtypes_start+1,iconstypes_start-1):
    line=lines[i]
    print line
    if line=='':
        continue
    if line[0]==';':
        print >> ouf, line
        continue

    data=line.split()
    aname1=data[0]
    aname2=data[1]
    sig=float(data[3])
    eps=float(data[4])
    print >> ouf, aname1,aname2,1,sig,eps
    print >> ouf, aname1,aname2+'_',1,rscale*sig,rscale*eps
    print >> ouf, aname1+'_',aname2,1,rscale*sig,rscale*eps
    print >> ouf, aname1+'_',aname2+'_',1,scale*sig,scale*eps

## write out stuff leading up to molecule data (unaltered)
print '####'
for i in range(ibondtypes_start,imoletype_start-1):
    if len(lines[i])>0:
        print >> ouf, lines[i]
        print lines[i]

print "####"
## scan through file until atom types are found
for istop in range(imoletype_start,100000):
    print >> ouf, lines[istop]
    if lines[istop]=='[ atoms ]':
        break
iatoms=istop

atom_types=[]
print >> ouf, '[ atoms ]'
for i in range(iatoms+1,iatoms+10000):

    if lines[i]=='':
        print >> ouf
        break
    if lines[i][0]==';':
        print >> ouf, lines[i]
        continue
    data=lines[i].split()
    serial=int(data[0])
    atype=data[1]
    ires=int(data[2])
    resname=data[3]
    aname=data[4]
    q=rscale*float(data[6])
    mass=float(data[7])
    atom_types.append(data[1])

    print >> ouf, "%6d  %5s  %6d  %3s  %5s  %6d %8.3f  %8.3f " % (serial,atype,ires,resname,aname,serial,q,mass)

ibonds=i+2
print >> ouf, '[ bonds ]'
for i in range(ibonds,ibonds+100000):
    print >> ouf, lines[i]
    if lines[i]=='[ dihedrals ]':
        break

idihedrals=i+1
for i in range(idihedrals,idihedrals+100000):

    if lines[i]=='':
        print >> ouf
        break

    if lines[i][0]==';':
        print >> ouf, lines[i]
        continue
    print lines[i]
    data=lines[i].split()
    i1=int(data[0])
    i2=int(data[1])
    i3=int(data[2])
    i4=int(data[3])
    type1=atom_types[i1-1][:-1]
    type2=atom_types[i2-1][:-1]
    type3=atom_types[i3-1][:-1]
    type4=atom_types[i4-1][:-1]

    phi=float(data[5])
    k=float(data[6])
    m=float(data[7])

    k*=scale
    print >> ouf, i1,i2,i3,i4,1,phi,k,m

iimp=i+2
print >> ouf, '[ dihedrals ]'
for i in range(iimp,iimp+10000):

    if lines[i]=='':
        print >> ouf
        break
    if lines[i][0]==';':
        print >> ouf, lines[i]
        continue
    data=lines[i].split()

    i1=int(data[0])
    i2=int(data[1])
    i3=int(data[2])
    i4=int(data[3])
    type1=atom_types[i1-1][:-1]
    type2=atom_types[i2-1][:-1]
    type3=atom_types[i3-1][:-1]
    type4=atom_types[i4-1][:-1]

    phi=float(data[5])
    k=float(data[6])
    k*=scale
    print >> ouf, i1,i2,i3,i4,2,phi,k

## print everything else unaltered to file

for line in lines[i:]:
    print >> ouf, line
